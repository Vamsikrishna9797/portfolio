...
function myMenuFunction(){
  ...
  srLeft.reveal('.contact-info,.section-text',{delay: 100})
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    counters.forEach(counter => {
      ...
      let start = fromValue;
      const step = (toValue - fromValue) / (duration / 10);
  
      function updateCounter() {
        start += step;
        if (start >= toValue) start = toValue;
        counter.textContent = Math.floor(start);
        if (start < toValue) setTimeout(updateCounter, 10);
      }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  progressBars.forEach(bar => {
      ...
      }, 500); // Start animation after page load
  });
});
